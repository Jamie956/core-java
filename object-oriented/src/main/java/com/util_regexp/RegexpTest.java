...
public class RegexpTest {
    ...
    public void match(String pattern, String... words) {
        ...
        for (String word : words) {
            if (word.matches(pattern)) {
                System.out.printf("[%s] match [%s]%n", word, pattern);
            } else {
                System.out.printf("[%s] not match [%s]%n", word, pattern);
            }
        }
    }
    ...
    public void replaceTest() {
        // 匹配html标签的表达式
        replace("<p>asdasdas</p><p>sdf45156321sd</p><p>bbb</p><p>cccc</p>", "<[^>]*>");

        // 匹配数字的表达式
        replace("<p>asdasdas</p><p>sdf45156321sd</p><p>bbb</p><p>cccc</p>", "\\d");

        // 匹配日期的表达式
        replace("CompanyNotice_2021-03-05.txt", "_\\d{4}-\\d{2}-\\d{2}");
    ...
    public void replace(String content, String regex) {
        System.out.println("------------------------------------------------------------------------");
        System.out.printf("待处理文本 [%s]%n", content);
        System.out.printf("使用正则表达式 [%s]%n", regex);
        String result = content.replaceAll(regex, "");
        System.out.printf("替换后结果 [%s]%n", result);
        System.out.println();
    }
    ...
    public void matcherTest() {
        //d+        找数字
        matcher("\\d+", "asa9jbjb1", "asa91jbjb232321");

        //找出文本的全部中文
        matcher("[\\u4E00-\\u9FA5]+", "asdasd阿松大啊实打实大苏打", "asa91jbjb232321");

        //找出指定开头x和结尾y的文本                 x.+.y
        matcher("x.+.y", "x1a23y111x23y2");

        //取 PIN= 为开头的内容                          (?<=PIN=).\S*
        matcher("(?<=PIN=).\\S*", "PIN=123");

        //取以=开头 以&结尾 取得的中间的内容            (?<==).*?(?=(&|$))
        matcher("(?<==).*?(?=(&|$))", "=asdasdnk&11", "=asdasdnk#");
    ...
    public void matcher(String pattern, String... items) {
        for (String item : items) {
            System.out.println("------------------------------------------");
            System.out.printf("待匹配的文本：[%s]%n", item);
            System.out.printf("使用表达式：[%s]%n", pattern);

            final Matcher matcher = Pattern.compile(pattern).matcher(item);
            while (matcher.find()) {
                System.out.printf("start=[%s] end=[%s] 匹配结果：[%s]%n", matcher.start(), matcher.end(), matcher.group());
            }
            System.out.println();
        }
    }
    ...
    // 截取字符串 由0到第n个中文
    @Test
    public void matchNChineseTest() {
        matchNChinese("as!@#@a9阿zsd松大jbjb的1123", 2);
        matchNChinese("阿松大asdasd啊实打实大苏打", 2);
        matchNChinese("阿松大啊实打实大苏打sadads", 2);
        matchNChinese("asdasd阿松大啊实打实大苏打", 3);
    }

    private void matchNChinese(String content, int n) {
        final Matcher matcher = Pattern.compile("[\\u4E00-\\u9FA5]+").matcher(content);
        int len = 0;
        int cur = -1;
        while (matcher.find()) {
        ...
        String result = content.substring(0, cur);
        System.out.println("------------------------------------");
        System.out.println("待处理文本：" + content);
